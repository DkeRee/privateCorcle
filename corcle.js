...
function stage5(){
  ...
  var lavaSpin1 = Matter.Bodies.rectangle(694, 495, 85, 20, {
    isStatic: true,
    rotationSpeed: 0.5,
    render: {
      fillStyle: 'transparent',
...
function stage6(){
  ...
  var tempBodies = [];
  var lavaBodies = [];
  var positiveSpeed = 0.1524;
  var negativeSpeed = -0.1524;
  ...
  var detectorBlock = Matter.Bodies.rectangle(695, 215, 155, 20, {
    isStatic: true,
    render: {
      fillStyle: 'transparent',
      strokeStyle: 'white',
  ...
  function moveSmallSpinnerLeft(){
    Matter.Body.translate(spinnerFive, {x: -5.6, y: 0});
  ...
  Matter.Events.on(engine, 'collisionStart', event => {
    ...
    for (var i = 0; i < lavaBodies.length; i++){
      if (lavaBodies[i] == pairs[0].bodyB){
        positiveSpeed = 0.1524;
        negativeSpeed = -0.1524;
        Matter.Body.setPosition(player, {x: 160, y: 80});
